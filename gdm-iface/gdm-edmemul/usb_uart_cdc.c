...
uint8_t fifo_tx_buf[0x200];
uint8_t rxbuf[CDC_DATA_SZ];
struct fifo_t usb_rx_fifo;
uint8_t fifo_rx_buf[0x200];
...
static void cdc_rxtx(usbd_device *dev, uint8_t event, uint8_t ep)
{
	...
	if (ep == CDC_RXD_EP) {
		...
		fifo_write_buf(&usb_rx_fifo, rxbuf, len);
...
//	wl = MIN(len, fifo_get_write_count(&tx_fifo));
	...
	fifo_write_buf(&tx_fifo, (void *)&pkt->sof, 1);
	fifo_write_buf(&tx_fifo, (void *)&pkt->cmd, 1);
	fifo_write_buf(&tx_fifo, (void *)&pkt->len, 2);
	fifo_write_buf(&tx_fifo, pkt->data, pkt->len);
	fifo_write_buf(&tx_fifo, (void *)&pkt->crc, 4);
...
void usb_uart_cdc_init(void)
{
	...
	fifo_init(&tx_fifo, fifo_tx_buf, sizeof(uint8_t), sizeof(fifo_tx_buf));
	fifo_init(&usb_rx_fifo, fifo_rx_buf, sizeof(uint8_t), sizeof(fifo_rx_buf));
